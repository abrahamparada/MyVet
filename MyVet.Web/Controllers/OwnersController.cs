    ...
    public class OwnersController : Controller
    {
        ...
        public IActionResult Index()
        {
            return View(_dataContext.Owners
                          .Include(o => o.User)  // Inner Join Owner && User
                          .Include(o => o.Pets));  // Inner Join Owner && Pets
        }
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var owner = await _dataContext.Owners
                .Include(o => o.User)  // Inner Join Owner && User
                .Include(o => o.Pets)  // Inner Join Owner && Pets
                .ThenInclude(p => p.PetType) // Inner  && Inner
                .Include(o => o.Pets)  // Inner Join Owner && Pets
                .ThenInclude(p => p.Histories)// Inner  && Inner
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            ...
            {
                ModelState.AddModelError(string.Empty, "You Can't Remove OWner");
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> AddPet(PetViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    path = await _imageHelper.UploadImageAsync(model.ImageFile); //Invoca Helper que carga imagen
                }
        ...
        
        {
            if(ModelState.IsValid)
            {
                var path = model.ImageUrl; // Inicializamos cargando la imagen que tiene cargada
                ...
                var pet = await _converterHelper.ToPetAsync(model, path, false);  // Existe mascota por lo que es un false para edit
                _dataContext.Pets.Update(pet);
                await _dataContext.SaveChangesAsync();
                return RedirectToAction($"Details/{model.OwnerId}");

            }
            model.PetTypes = _combosHelper.GetComboPetTypes(); // Si Existe error recarga nuevamente el Combo
            return View(model);
